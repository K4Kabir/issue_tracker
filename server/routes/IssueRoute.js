...
const issueRouteWithIo = (io) => {
  ...
  router.post("/updateIssue", upload.single("image"), async (req, res) => {
    const { title, description, projectId, assignedUsers, id, status } =
      req.body;
  ...
  router.post("/deleteIssue", async (req, res) => {
    ...
    if (deleted) {
      const afterDelete = await prisma.issue.findMany();
      io.emit("IssueDeleted", { message: afterDelete });
...
module.exports = issueRouteWithIo;
